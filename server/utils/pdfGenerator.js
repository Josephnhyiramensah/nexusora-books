...
function drawFooter(doc, pageNum, showBranding = true) {
  // y must stay far enough above the A4 bottom margin (842 height - 50 margin =
  // 792) that an 8pt line does not spill past it — otherwise pdfkit auto-adds a
  // blank page. lineBreak:false stops the footer text from ever wrapping into a
  // new page as well.
  const y = 760;
  drawLine(doc, y);
  doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica');
  if (showBranding) {
    doc.text('Generated by Nexusora Books · Powered by Nexusora Technologies', 50, y + 8, { align: 'left', lineBreak: false });
  }
  doc.text(`Page ${pageNum}`, 50, y + 8, { align: 'right', width: 495, lineBreak: false });
}
