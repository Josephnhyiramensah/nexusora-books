...
function drawFooter(doc, pageNum) {
  ...
  doc.fillColor(COLORS.gray).fontSize(8).font('Helvetica')
    .text('Generated by Nexusora Books · Powered by Nexusora Technologies', 50, y + 8, { align: 'left' })
    .text(`Page ${pageNum}`, 50, y + 8, { align: 'right', width: 495 });
}

// ─── Customer Statement ───────────────────────────────────────────────────────
async function generateCustomerStatement({ customer, invoices, tenantSettings, companyName }) {
...
// ─── Invoice PDF ──────────────────────────────────────────────────────────────
async function generateInvoicePDF({ invoice, customer, tenantSettings, companyName }) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ size: 'A4', margin: 50 });
      const chunks = [];
